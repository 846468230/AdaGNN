...
def gnn_architecture_save(args, best_alpha_model_list):
    data_name = args.dataset
    seed = args.seed
    if not os.path.exists(logger_path + '/' + str(data_name)):
        os.makedirs(logger_path + '/' + str(data_name))

    fp = f"{logger_path}/{data_name}/{data_name}_gnn_logger_seed_{seed}.txt"
    with open(fp, "w") as f:
...
def gnn_architecture_load(args, gnn_layers):
...
def gnn_architecture_merge(best_alpha_model_split, gnn_layers):
    ...
    for layer in range(gnn_layers):
        attention = best_alpha_model_split[layer * 4 + 0]
        aggregation = best_alpha_model_split[layer * 4 + 1]
        hidden_dimension = str(best_alpha_model_split[layer * 4 + 2])
        convolution = '-'.join([attention, aggregation, hidden_dimension])
        best_alpha_model.append(convolution)
        activation = best_alpha_model_split[layer * 4 + 3]
...
def gnn_architecture_split(best_alpha_model, gnn_layers):
    best_alpha_model_split = []

    for layer in range(gnn_layers):
        convolution = best_alpha_model[layer * 2 + 0]
        ...
        activation = best_alpha_model[layer * 2 + 1]
